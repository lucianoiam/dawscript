...
def register_service(name, service_type, port, address):
   ...
   if zc_instance is not None:
      global serv_info
      serv_info = ServiceInfo(f'{service_type}.local.',
         f'{name}.{service_type}.local.', addresses=[address], port=port)
      zc_instance.register_service(serv_info)
   else:
      global subproc
      if _is_command_in_path('dns-sd'):
         subproc = subprocess.Popen(['dns-sd', '-R', name, service_type, '.',
            str(port), address_str])
      elif _is_command_in_path('avahi-publish'):
         subproc = subprocess.Popen(['avahi-publish', '-s', name,
            service_type, str(port), address_str])
      else:
